defmodule RiakTS.Protocol do
  @moduledoc false

  #TODO check this module carefully

  alias RiakTS.{Types, Query, Stream}

  import RiakTS.Messages
  ...
  def connect(opts) do
    ...
    extensions = custom ++ RiakTS.Utils.default_extensions(ext_opts)
    ssl?       = opts[:ssl] || false
    ...
    s = %__MODULE__{timeout: timeout, riakts: :idle,
  ...
  @spec ping(state) ::
    {:ok, state} | {:disconnect, RiakTS.Error.t, state}
  def ping(%{riakts: :transaction, transactions: :strict} = s) do
  ...
  @spec checkout(state) ::
    {:ok, state} | {:disconnect, RiakTS.Error.t, state}
  def checkout(%{riakts: :transaction, transactions: :strict} = s) do
    sync_error(s, :transaction)
  end
  ...
  @spec checkin(state) ::
  {:ok, state} | {:disconnect, RiakTS.Error.t, state}
  def checkin(%{riakts: :transaction, transactions: :strict} = s) do
    sync_error(s, :transaction)
  end
  ...
  @spec handle_prepare(RiakTS.Query.t, Keyword.t, state) ::
    {:ok, RiakTS.Query.t, state} |
    {:error, ArgumentError.t | RuntimeError.t, state} |
    {:error | :disconnect, RiakTS.Query.t, state}
  def handle_prepare(query, _, %{riakts: {_, _}} = s) do
  ...
  @spec handle_execute(RiakTS.Parameters.t, nil, Keyword.t, state) ::
    {:ok, %{binary => binary}, state} |
    {:error, RiakTS.Errpr.t, state}
  def handle_execute(%RiakTS.Parameters{}, nil, _, s) do
    %{parameters: parameters} = s
    case RiakTS.Parameters.fetch(parameters) do
      {:ok, parameters} ->
        {:ok, parameters, s}
      :error ->
        {:error, %RiakTS.Error{message: "parameters not available"}, s}
    end
  end

  @spec handle_execute(RiakTS.Stream.t | RiakTS.Query.t, list, Keyword.t, state) ::
    {:ok, RiakTS.Result.t, state} |
    {:error, ArgumentError.t, state} |
    {:error | :disconnect, RuntimeError.t, RiakTS.Error.t, state}
  ...
  @spec handle_close(RiakTS.Query.t | RiakTS.Stream.t, Keyword.t, state) ::
    {:ok, RiakTS.Result.t, state} |
    {:error, ArgumentError.t, state} |
    {:error | :disconnect, RuntimeError.t | RiakTS.Error.t, state}
  def handle_close(%Stream{ref: ref} = stream, _, %{riakts: {_, ref}} = s) do
    msg = "postgresql protocol can not halt copying from database for " <>
      inspect(stream)
    err = RuntimeError.exception(message: msg)
    {:disconnect, err, s}
  end
  def handle_close(query, _, %{riakts: {_, _}} = s) do
  ...
  def handle_close(query, opts, s) do
    ...
    status = %{notify: notify(opts), mode: mode(opts), sync: :sync}
    res = %RiakTS.Result{command: :close, connection_id: connection_id}
  ...
  @spec handle_begin(Keyword.t, state) ::
    ...
    {:error | :disconnect, RiakTS.Error.t, state}
  def handle_begin(_, %{riakts: {_, _}} = s) do
    lock_error(s, :begin)
  end
  ...
  @spec handle_commit(Keyword.t, state) ::
    ...
    {:disconnect, RuntimeError.t, state} |
    {:error | :disconnect, RiakTS.Error.t, state}
  def handle_commit(_, %{riakts: {_, _}} = s) do
    lock_error(s, :commit)
  end
  def handle_commit(opts, %{riakts: riakts} = s) do
    case Keyword.get(opts, :mode, :transaction) do
      :transaction ->
        statement = "COMMIT"
        handle_transaction(statement, :idle, :commit, opts, s)
      :savepoint when riakts == :failed ->
  ...
  @spec handle_rollback(Keyword.t, state) ::
    ...
    {:disconnect, RuntimeError.t, state} |
    {:error | :disconnect, RiakTS.Error.t, state}
  def handle_rollback(_, %{riakts: {_, _}} = s) do
  ...
  @spec handle_info(any, Keyword.t, state) ::
    {:ok, state} | {:error | :disconnect, RiakTS.Error.t, state}
  ...
  def handle_info({:tcp_closed, sock}, _, %{sock: {:gen_tcp, sock}} = s) do
    err = RiakTS.Error.exception(tag: :tcp, action: "async recv", reason: :closed)
    {:disconnect, err, s}
  end
  def handle_info({:tcp_error, sock, reason}, _, %{sock: {:gen_tcp, sock}} = s) do
    err = RiakTS.Error.exception(tag: :tcp, action: "async recv", reason: reason)
    {:disconnect, err, s}
  end
  ...
  def handle_info({:ssl_closed, sock}, _, %{sock: {:ssl, sock}} = s) do
    err = RiakTS.Error.exception(tag: :ssl, action: "async recv", reason: :closed)
    {:disconnect, err, s}
  end
  def handle_info({:ssl_error, sock, reason}, _, %{sock: {:ssl, sock}} = s) do
    err = RiakTS.Error.exception(tag: :ssl, action: "async recv", reason: reason)
    {:disconnect, err, s}
  end
  ...
  defp connect(host, port, sock_opts, %{timeout: timeout} = s) do
    ...
    case :gen_tcp.connect(host, port, sock_opts ++ @sock_opts, timeout) do
      ...
      {:ok, sock} ->
        ...
        {:ok, %{s | sock: {:gen_tcp, sock}}}
      {:error, reason} ->
        {:error, RiakTS.Error.exception(tag: :tcp, action: "connect", reason: reason)}
    end
  end
  ...
  defp handshake(%{timeout: timeout, sock: {:gen_tcp, sock}} = s,status) do
    ...
    case do_handshake(s, status) do
      {:ok, %{parameters: parameters} = s} ->
        ...
        ref = RiakTS.Parameters.insert(parameters)
        {:ok, %{s | parameters: ref}}
      {:disconnect, err, s} ->
        cancel_handshake_timer(timer)
        disconnect(err, s)
  ...
  defp ssl_recv(%{sock: {:gen_tcp, sock}} = s, status) do
    case :gen_tcp.recv(sock, 1, :infinity) do
      ...
      {:ok, <<?N>>} ->
        disconnect(s, %RiakTS.Error{message: "ssl not available"}, "")
      {:error, reason} ->
        err = RiakTS.Error.exception(tag: :tcp, action: "recv", reason: reason)
        disconnect(s, err, "")
    end
  end
  ...
  defp ssl_connect(%{sock: {:gen_tcp, sock}, timeout: timeout} = s, status) do
    case :ssl.connect(sock, status.opts[:ssl_opts] || [], timeout) do
      ...
      {:error, reason} ->
        err = RiakTS.Error.exception(tag: :ssl, action: "connect", reason: reason)
        disconnect(s, err, "")
    end
  end
  ...
  defp auth_recv(s, status, buffer) do
    case msg_recv(s, :infinity, buffer) do
      ...
      {:ok, msg_error(fields: fields), buffer} ->
        disconnect(s, RiakTS.Error.exception(riakts: fields), buffer)
      {:disconnect, _, _} = dis ->
        dis
    end
  end
  ...
  defp bootstrap(s, %{types_key: types_key} = status, buffer) do
    case RiakTS.TypeServer.fetch(types_key) do
      {:lock, ref, table} ->
  ...
  defp bootstrap_send(%{parameters: parameters} = s, status, buffer) do
    version = parameters["server_version"] |> RiakTS.Utils.parse_version
  ...
  defp bootstrap_recv(s, status, buffer) do
    case msg_recv(s, :infinity, buffer) do
      {:ok, msg_row_desc(), buffer} ->
        bootstrap_recv(s, status, [], buffer)
      {:ok, msg_error(fields: fields), buffer} ->
        err = RiakTS.Error.exception(riakts: fields)
  ...
  defp bootstrap_sync_recv(s, status, buffer) do
    case msg_recv(s, :infinity, buffer) do
      ...
      {:ok, msg_ready(status: riakts), buffer} ->
        sync_error(s, riakts, buffer)
      {:ok, msg, buffer} ->
  ...
  defp simple_recv(%{timeout: timeout} = s, status, buffer) do
    ## simple queries here are only done by RiakTS.Notifications processes
  ...
  defp describe_recv(s, status, query, buffer) do
    case msg_recv(s, :infinity, buffer) do
      ...
      {:ok, msg_error(fields: fields), buffer} ->
        sync_recv(s, status, RiakTS.Error.exception(riakts: fields), buffer)
      {:ok, msg, buffer} ->
  ...
  defp execute(%{riakts: {_, _ref}} = s, %Query{} = query) do
  ...
  defp execute(%{riakts: {_, ref}}, %Stream{ref: ref, state: state} = stream) do
    case state do
      :copy_out ->
        {:copy_out, stream}
      :copy_done ->
  ...
  defp execute(%{riakts: {_, _ref}} = s, %Stream{} = stream) do
  ...
  defp execute(%{riakts: {_, ref}}, %RiakTS.CopyData{ref: ref}) do
    :copy_data
  end
  defp execute(%{riakts: {_, _ref}} = s, %RiakTS.CopyData{} = copy_data) do
  ...
  defp bind_send(s, status, stream, query, params, buffer) do
    ...
    res = %RiakTS.Result{command: :bind, connection_id: connection_id}
  ...
  defp bind_error(s, status, query, fields, buffer) do
    err = RiakTS.Error.exception(riakts: fields)
    _ = if err.riakts.code == :invalid_sql_statement_name do
      Logger.error fn() ->
  ...
  defp execute_recv(s, status, query, buffer) do
    case msg_recv(s, :infinity, buffer) do
      ...
      {:ok, msg_empty_query(), buffer} ->
        sync_recv(s, status, %RiakTS.Result{num_rows: 0}, buffer)
      {:ok, msg_error(fields: fields), buffer} ->
        err = RiakTS.Error.exception(riakts: fields)
        sync_recv(s, status, err, buffer)
  ...
  defp complete(s, status, %Query{} = query, rows, tag, buffer) do
    ...
    result = %RiakTS.Result{command: command, num_rows: nrows || 0,
  ...
  defp complete(s, status, stream, rows, tag, buffer) do
    %RiakTS.Stream{query: query, num_rows: previous_nrows} = stream
    %{connection_id: connection_id} = s
    {command, nrows} = decode_tag(tag)
    %Query{columns: cols} = query
    # Fix for PostgreSQL 8.4 (doesn't include number of selected rows in tag)
  ...
  defp suspend(s, status, stream, rows, buffer) do
    %{connection_id: connection_id} = s
    %RiakTS.Stream{query: %Query{columns: cols}} = stream

    result = %RiakTS.Result{command: :stream, num_rows: :stream,
  ...
  defp copy_out_recv(s, _, stream, max_rows, acc, max_rows, buffer) do
    ...
    %{riakts: riakts, connection_id: connection_id} = s
    result = %RiakTS.Result{command: :copy_stream, num_rows: :copy_stream,
      rows: acc, columns: nil, connection_id: connection_id}
    ok(s, result, {riakts, ref}, buffer)
  end
  ...
  defp copy_in_ready(s, _status, stream, buffer) do
    %{connection_id: connection_id, riakts: riakts} = s
    result = %RiakTS.Result{connection_id: connection_id, command: :copy_stream,
  ...
  defp copy_data(s, _status, data, buffer) do
    case do_send(s, data, buffer) do
      :ok ->
        %{connection_id: connection_id, riakts: riakts} = s
        result = %RiakTS.Result{connection_id: connection_id,
                                  command: :copy_stream, rows: nil,
                                  num_rows: :copy_stream}
        ok(s, result, riakts, buffer)
      {:disconnect, _, _} = dis ->
        dis
    end
  end
  ...
  defp handle_transaction(name, next_riakts, cmd, opts, s) do
    %{connection_id: connection_id, buffer: buffer} = s
    status = %{notify: notify(opts), mode: :transaction, sync: :sync}
    res = %RiakTS.Result{command: cmd, connection_id: connection_id}
    transaction_send(%{s | buffer: nil}, status, name, next_riakts, res, buffer)
  end

  defp transaction_send(s, status, statement, next_riakts, res, buffer) do
    msgs = transaction_msgs(s, [statement, :sync])
    case msg_send(s, msgs, buffer) do
      :ok ->
        transaction_recv(s, status, next_riakts, res, buffer)
      {:disconnect, _, _} = dis ->
        dis
    end
  end
  ...
  defp transaction_recv(s, status, next_riakts, res, buffer) do
    %{transactions: transactions} = s
    case msg_recv(s, :infinity, buffer) do
      {:ok, msg_ready(status: riakts), buffer} when transactions == :naive ->
        ok(s, res, riakts, buffer)
      {:ok, msg_ready(status: ^next_riakts), buffer} ->
        ok(s, res, next_riakts, buffer)
      {:ok, msg_ready(status: riakts), buffer} ->
        sync_error(s, riakts, buffer)
      {:ok, msg_parse_complete(), buffer} ->
        transaction_recv(s, status, next_riakts, res, buffer)
  ...
  defp savepoint_recv(s, status, res, buffer) do
    %{riakts: riakts, transactions: transactions} = s
    case msg_recv(s, :infinity, buffer) do
      {:ok, msg_parse_complete(), buffer} ->
        savepoint_recv(s, status, res, buffer)
      {:ok, msg_bind_complete(), buffer} ->
      ...
      {:ok, msg_command_complete(), buffer} ->
        savepoint_recv(s, status, res, buffer)
      {:ok, msg_ready(status: :idle), buffer}
      when riakts == :transaction and transactions == :strict ->
        sync_error(s, :idle, buffer)
      {:ok, msg_ready(status: :transaction), buffer}
      when riakts == :idle and transactions == :strict ->
  ...
  defp handle_msg(s, _, msg_parameter(name: name, value: value)) do
    ...
    cond do
      is_reference(parameters) ->
        _ = RiakTS.Parameters.put(parameters, name, value)
        s
      is_map(parameters) ->
  ...
  defp ok(s, %RiakTS.Result{} = res, riakts, buffer) do
    {:ok, res, %{s | riakts: riakts, buffer: buffer}}
  end
  defp ok(s, %RiakTS.Query{} = query, riakts, buffer) do
    {:ok, query, %{s | riakts: riakts, buffer: buffer}}
   end
  defp ok(s, %RiakTS.Error{} = err, riakts, buffer) do
    %{connection_id: connection_id} = s
    err = %{err | connection_id: connection_id}
    {:error, err, %{s | riakts: riakts, buffer: buffer}}
  end
  defp ok(s, :active_once, riakts, buffer) do
    activate(%{s | riakts: riakts}, buffer)
  end
  defp ok(s, nil, riakts, buffer) do
    {:ok, %{s | riakts: riakts, buffer: buffer}}
  end

  defp disconnect(s, tag, action, reason, buffer) do
    err = RiakTS.Error.exception(tag: tag, action: action, reason: reason)
    disconnect(s, err, buffer)
  end

  defp disconnect(%{connection_id: connection_id} = s, %RiakTS.Error{} = err, buffer) do
  ...
  defp do_sync_recv(s, status, res, buffer) do
    %{riakts: riakts, transactions: transactions, timeout: timeout} = s
    case msg_recv(s, timeout, buffer) do
      {:ok, msg_ready(status: :idle), buffer}
      when riakts == :transaction and transactions == :strict ->
        sync_error(s, :idle, buffer)
  ...
  defp sync_error(s, riakts, buffer) do
    sync_error(%{s | buffer: buffer}, riakts)
  end

  defp sync_error(s, riakts) do
    err = %RiakTS.Error{message: "unexpected riakts status: #{riakts}"}
    {:disconnect, err, s}
  end
