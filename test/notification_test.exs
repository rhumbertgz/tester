defmodule NotificationTest do
  ...
  alias RiakTS, as: P
  alias RiakTS.Notifications, as: PN
  ...
  test "listening, notify, then receive (with payload)", context do
    ...
    assert {:ok, %RiakTS.Result{command: :notify}} = P.query(context.pid, "NOTIFY channel, 'hello'", [])
  ...
  test "listening, notify, then receive (without payload)", context do
    ...
    assert {:ok, %RiakTS.Result{command: :notify}} = P.query(context.pid, "NOTIFY channel", [])
    receiver_pid = context.pid_ps
    ref = ref
