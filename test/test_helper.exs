...
defmodule RiakTS.TestHelper do
  defmacro query(stat, params, opts \\ []) do
    quote do
      case RiakTS.query(var!(context)[:pid], unquote(stat),
                                     unquote(params), unquote(opts)) do
        {:ok, %RiakTS.Result{rows: nil}} -> :ok
        {:ok, %RiakTS.Result{rows: rows}} -> rows
        {:error, %RiakTS.Error{} = err} -> err
      end
    end
  end
  ...
  defmacro prepare(name, stat, opts \\ []) do
    quote do
      case RiakTS.prepare(var!(context)[:pid], unquote(name),
                                     unquote(stat), unquote(opts)) do
        {:ok, %RiakTS.Query{} = query} -> query
        {:error, %RiakTS.Error{} = err} -> err
      end
    end
  end
  ...
  defmacro execute(query, params, opts \\ []) do
    quote do
      case RiakTS.execute(var!(context)[:pid], unquote(query),
                                       unquote(params), unquote(opts)) do
        {:ok, %RiakTS.Result{rows: nil}} -> :ok
        {:ok, %RiakTS.Result{rows: rows}} -> rows
        {:error, %RiakTS.Error{} = err} -> err
  ...
  defmacro stream(query, params, opts \\ []) do
    quote do
      RiakTS.stream(var!(conn), unquote(query), unquote(params), unquote(opts))
    end
  end

  defmacro close(query, opts \\ []) do
    quote do
      case RiakTS.close(var!(context)[:pid], unquote(query),
                                     unquote(opts)) do
        :ok -> :ok
        {:error, %RiakTS.Error{} = err} -> err
      end
  ...
  defmacro transaction(fun, opts \\ []) do
    quote do
      RiakTS.transaction(var!(context)[:pid], unquote(fun),
                                      unquote(opts))
    end
  end
