defmodule LoginTest do
  ...
  test "login cleartext password" do
    ...
    assert {:ok, %RiakTS.Result{}} = P.query(pid, "SELECT 123", [])
  end
  ...
  test "login cleartext password failure" do
    ...
    capture_log fn ->
      assert {:ok, pid} = P.start_link(opts)
      assert_receive {:EXIT, ^pid, {%RiakTS.Error{postgres: %{code: code}}, [_|_]}}
  ...
  test "non existant database" do
    ...
    capture_log fn ->
      opts = [ database: "doesntexist", sync_connect: true ,
               backoff_type: :stop ]
      assert {:error, {%RiakTS.Error{postgres: %{code: :invalid_catalog_name}}, [_|_]}} =
             P.start_link(opts)

      assert_receive {:EXIT, _, {%RiakTS.Error{postgres: %{code: :invalid_catalog_name}}, [_|_]}}
    end

    capture_log fn ->
      opts = [ database: "doesntexist", backoff_type: :stop ]
      {:ok, pid} = P.start_link(opts)
      assert_receive {:EXIT, ^pid, {%RiakTS.Error{postgres: %{code: :invalid_catalog_name}}, [_|_]}}
    end
  end
  ...
  test "non-existent domain" do
    ...
    capture_log fn ->
      assert {:ok, pid} = P.start_link(opts)
      assert_receive {:EXIT, ^pid, {%RiakTS.Error{message: message}, [_|_]}}
  ...
  test "after connect function run" do
    ...
    assert_receive {:ok, %RiakTS.Result{}}
  end
