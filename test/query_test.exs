defmodule QueryTest do
  ...
  test "decode time", context do
    assert [[%RiakTS.Time{hour: 0, min: 0, sec: 0, usec: 0}]] =
           query("SELECT time '00:00:00'", [])
    assert [[%RiakTS.Time{hour: 1, min: 2, sec: 3, usec: 0}]] =
           query("SELECT time '01:02:03'", [])
    assert [[%RiakTS.Time{hour: 23, min: 59, sec: 59, usec: 0}]] =
           query("SELECT time '23:59:59'", [])
    assert [[%RiakTS.Time{hour: 4, min: 5, sec: 6, usec: 0}]] =
           query("SELECT time '04:05:06 PST'", [])

    assert [[%RiakTS.Time{hour: 0, min: 0, sec: 0, usec: 123000}]] =
           query("SELECT time '00:00:00.123'", [])
    assert [[%RiakTS.Time{hour: 0, min: 0, sec: 0, usec: 123456}]] =
           query("SELECT time '00:00:00.123456'", [])
    assert [[%RiakTS.Time{hour: 1, min: 2, sec: 3, usec: 123456}]] =
           query("SELECT time '01:02:03.123456'", [])
  end

  test "decode date", context do
    assert [[%RiakTS.Date{year: 1, month: 1, day: 1}]] =
           query("SELECT date '0001-01-01'", [])
    assert [[%RiakTS.Date{year: 1, month: 2, day: 3}]] =
           query("SELECT date '0001-02-03'", [])
    assert [[%RiakTS.Date{year: 2013, month: 9, day: 23}]] =
           query("SELECT date '2013-09-23'", [])
  end

  test "decode timestamp", context do
    assert [[%RiakTS.Timestamp{year: 2001, month: 1, day: 1, hour: 0, min: 0, sec: 0, usec: 0}]] =
           query("SELECT timestamp '2001-01-01 00:00:00'", [])
    assert [[%RiakTS.Timestamp{year: 2013, month: 9, day: 23, hour: 14, min: 4, sec: 37, usec: 123000}]] =
           query("SELECT timestamp '2013-09-23 14:04:37.123'", [])
    assert [[%RiakTS.Timestamp{year: 2013, month: 9, day: 23, hour: 14, min: 4, sec: 37, usec: 0}]] =
           query("SELECT timestamp '2013-09-23 14:04:37 PST'", [])
    assert [[%RiakTS.Timestamp{year: 1, month: 1, day: 1, hour: 0, min: 0, sec: 0, usec: 123456}]] =
           query("SELECT timestamp '0001-01-01 00:00:00.123456'", [])

  end

  test "decode interval", context do
    assert [[%RiakTS.Interval{months: 0, days: 0, secs: 0}]] =
           query("SELECT interval '0'", [])
    assert [[%RiakTS.Interval{months: 100, days: 0, secs: 0}]] =
           query("SELECT interval '100 months'", [])
    assert [[%RiakTS.Interval{months: 0, days: 100, secs: 0}]] =
           query("SELECT interval '100 days'", [])
    assert [[%RiakTS.Interval{months: 0, days: 0, secs: 100}]] =
           query("SELECT interval '100 secs'", [])
    assert [[%RiakTS.Interval{months: 14, days: 40, secs: 10920}]] =
           query("SELECT interval '1 year 2 months 40 days 3 hours 2 minutes'", [])
  end

  test "decode point", context do
    assert [[%RiakTS.Point{x: -97.5, y: 100.1}]] == query("SELECT point(-97.5, 100.1)::point", [])
  end

  test "encode point", context do
    assert [[%RiakTS.Point{x: -97.0, y: 100.0}]] == query("SELECT $1::point", [%RiakTS.Point{x: -97, y: 100}])
  end
  ...
  test "decode range", context do
    assert [[%RiakTS.Range{lower: 2, upper: 5, lower_inclusive: true, upper_inclusive: false}]] =
           query("SELECT '(1,5)'::int4range", [])
    assert [[%RiakTS.Range{lower: 1, upper: 7, lower_inclusive: true, upper_inclusive: false}]] =
           query("SELECT '[1,6]'::int4range", [])
    assert [[%RiakTS.Range{lower: nil, upper: 5, lower_inclusive: false, upper_inclusive: false}]] =
           query("SELECT '(,5)'::int4range", [])
    assert [[%RiakTS.Range{lower: 1, upper: nil, lower_inclusive: true, upper_inclusive: false}]] =
           query("SELECT '[1,)'::int4range", [])
    assert [[%RiakTS.Range{lower: nil, upper: nil, lower_inclusive: false, upper_inclusive: false}]] =
           query("SELECT '(,)'::int4range", [])
    assert [[%RiakTS.Range{lower: nil, upper: nil, lower_inclusive: false, upper_inclusive: false}]] =
           query("SELECT '[,]'::int4range", [])

    assert [[%RiakTS.Range{lower: 3, upper: 8, lower_inclusive: true, upper_inclusive: false}]] =
           query("SELECT '(2,8)'::int8range", [])

    assert [[%RiakTS.Range{lower: Decimal.new("1.2"), upper: Decimal.new("3.4"), lower_inclusive: false, upper_inclusive: false}]] ==
           query("SELECT '(1.2,3.4)'::numrange", [])

    assert [[%RiakTS.Range{lower: %RiakTS.Date{year: 2014, month: 1, day: 1}, upper: %RiakTS.Date{year: 2014, month: 12, day: 31}}]] =
           query("SELECT '[2014-1-1,2014-12-31)'::daterange", [])
    assert [[%RiakTS.Range{lower: nil, upper: %RiakTS.Date{year: 2014, month: 12, day: 31}}]] =
           query("SELECT '(,2014-12-31)'::daterange", [])
    assert [[%RiakTS.Range{lower: %RiakTS.Date{year: 2014, month: 1, day: 2}, upper: nil}]] =
           query("SELECT '(2014-1-1,]'::daterange", [])
  end

  @tag min_pg_version: "9.0"
  test "decode network types", context do
    assert [[%RiakTS.INET{address: {127, 0, 0, 1}}]] =
           query("SELECT '127.0.0.1'::inet", [])
    assert [[%RiakTS.INET{address: {8193, 43981, 0, 0, 0, 0, 0, 0}}]] =
           query("SELECT '2001:abcd::'::inet", [])
    assert [[%RiakTS.CIDR{address: {127, 0, 0, 1}, netmask: 32}]] =
           query("SELECT '127.0.0.1/32'::cidr", [])
    assert [[%RiakTS.CIDR{address: {8193, 43981, 0, 0, 0, 0, 0, 0}, netmask: 128}]] =
           query("SELECT '2001:abcd::/128'::cidr", [])
    assert [[%RiakTS.MACADDR{address: {8, 1, 43, 5, 7, 9}}]] =
  ...
  test "encoding oids as binary fails with a helpful error message", context do
    assert %RiakTS.Error{message: message} = catch_error(query("select $1::regclass;", ["pg_type"]))
  ...
  test "fail on encoding wrong value", context do
    ...
    assert message =~ "RiakTS expected an integer in -2147483648..2147483647 that can be encoded/cast to type \"int4\""
  end
  ...
  test "encode date", context do
    assert [[%RiakTS.Date{year: 1, month: 1, day: 1}]] =
           query("SELECT $1::date", [%RiakTS.Date{year: 1, month: 1, day: 1}])
    assert [[%RiakTS.Date{year: 1, month: 2, day: 3}]] =
           query("SELECT $1::date", [%RiakTS.Date{year: 1, month: 2, day: 3}])
    assert [[%RiakTS.Date{year: 2013, month: 9, day: 23}]] =
           query("SELECT $1::date", [%RiakTS.Date{year: 2013, month: 9, day: 23}])
    assert [[%RiakTS.Date{year: 1999, month: 12, day: 31}]] =
           query("SELECT $1::date", [%RiakTS.Date{year: 1999, month: 12, day: 31}])
    assert [[%RiakTS.Date{year: 1999, month: 12, day: 31}]] =
           query("SELECT $1::date", [%RiakTS.Date{year: 1999, month: 12, day: 31}])
  end

  test "encode time", context do
    assert [[%RiakTS.Time{hour: 0, min: 0, sec: 0}]] =
           query("SELECT $1::time", [%RiakTS.Time{hour: 0, min: 0, sec: 0}])
    assert [[%RiakTS.Time{hour: 1, min: 2, sec: 3}]] =
           query("SELECT $1::time", [%RiakTS.Time{hour: 1, min: 2, sec: 3}])
    assert [[%RiakTS.Time{hour: 23, min: 59, sec: 59}]] =
           query("SELECT $1::time", [%RiakTS.Time{hour: 23, min: 59, sec: 59}])
    assert [[%RiakTS.Time{hour: 4, min: 5, sec: 6, usec: 123456}]] =
           query("SELECT $1::time", [%RiakTS.Time{hour: 4, min: 5, sec: 6, usec: 123456}])
  end

  test "encode timestamp", context do
    assert [[%RiakTS.Timestamp{year: 1, month: 1, day: 1, hour: 0, min: 0, sec: 0}]] =
      query("SELECT $1::timestamp", [%RiakTS.Timestamp{year: 1, month: 1, day: 1, hour: 0, min: 0, sec: 0}])
    assert [[%RiakTS.Timestamp{year: 2013, month: 9, day: 23, hour: 14, min: 4, sec: 37}]] =
      query("SELECT $1::timestamp", [%RiakTS.Timestamp{year: 2013, month: 9, day: 23, hour: 14, min: 4, sec: 37}])
    assert [[%RiakTS.Timestamp{year: 1, month: 1, day: 1, hour: 0, min: 0, sec: 0, usec: 123456}]] =
      query("SELECT $1::timestamp", [%RiakTS.Timestamp{year: 1, month: 1, day: 1, hour: 0, min: 0, sec: 0, usec: 123456}])
  end

  test "encode interval", context do
    assert [[%RiakTS.Interval{months: 0, days: 0, secs: 0}]] =
           query("SELECT $1::interval", [%RiakTS.Interval{months: 0, days: 0, secs: 0}])
    assert [[%RiakTS.Interval{months: 100, days: 0, secs: 0}]] =
           query("SELECT $1::interval", [%RiakTS.Interval{months: 100, days: 0, secs: 0}])
    assert [[%RiakTS.Interval{months: 0, days: 100, secs: 0}]] =
           query("SELECT $1::interval", [%RiakTS.Interval{months: 0, days: 100, secs: 0}])
    assert [[%RiakTS.Interval{months: 0, days: 0, secs: 100}]] =
           query("SELECT $1::interval", [%RiakTS.Interval{months: 0, days: 0, secs: 100}])
    assert [[%RiakTS.Interval{months: 14, days: 40, secs: 10920}]] =
           query("SELECT $1::interval", [%RiakTS.Interval{months: 14, days: 40, secs: 10920}])
  end
  ...
  test "encode range", context do
    assert [[%RiakTS.Range{lower: 1, upper: 4, lower_inclusive: true, upper_inclusive: false}]] =
           query("SELECT $1::int4range", [%RiakTS.Range{lower: 1, upper: 3, lower_inclusive: true, upper_inclusive: true}])
    assert [[%RiakTS.Range{lower: nil, upper: 6, lower_inclusive: false, upper_inclusive: false}]] =
           query("SELECT $1::int4range", [%RiakTS.Range{lower: nil, upper: 5, lower_inclusive: false, upper_inclusive: true}])
    assert [[%RiakTS.Range{lower: 3, upper: nil, lower_inclusive: true, upper_inclusive: false}]] =
           query("SELECT $1::int4range", [%RiakTS.Range{lower: 3, upper: nil, lower_inclusive: true, upper_inclusive: true}])
    assert [[%RiakTS.Range{lower: 4, upper: 5, lower_inclusive: true, upper_inclusive: false}]] =
           query("SELECT $1::int4range", [%RiakTS.Range{lower: 3, upper: 5, lower_inclusive: false, upper_inclusive: false}])

    assert [[%RiakTS.Range{lower: 1, upper: 4, lower_inclusive: true, upper_inclusive: false}]] =
           query("SELECT $1::int8range", [%RiakTS.Range{lower: 1, upper: 3, lower_inclusive: true, upper_inclusive: true}])

    assert [[%RiakTS.Range{lower: Decimal.new("1.2"), upper: Decimal.new("3.4"), lower_inclusive: true, upper_inclusive: true}]] ==
           query("SELECT $1::numrange", [%RiakTS.Range{lower: Decimal.new("1.2"), upper: Decimal.new("3.4"), lower_inclusive: true, upper_inclusive: true}])

    assert [[%RiakTS.Range{lower: %RiakTS.Date{year: 2014, month: 1, day: 1}, upper: %RiakTS.Date{year: 2015, month: 1, day: 1}}]] =
           query("SELECT $1::daterange", [%RiakTS.Range{lower: %RiakTS.Date{year: 2014, month: 1, day: 1}, upper: %RiakTS.Date{year: 2014, month: 12, day: 31}}])
    assert [[%RiakTS.Range{lower: nil, upper: %RiakTS.Date{year: 2015, month: 1, day: 1}}]] =
           query("SELECT $1::daterange", [%RiakTS.Range{lower: nil, upper: %RiakTS.Date{year: 2014, month: 12, day: 31}}])
    assert [[%RiakTS.Range{lower: %RiakTS.Date{year: 2014, month: 1, day: 1}, upper: nil}]] =
           query("SELECT $1::daterange", [%RiakTS.Range{lower: %RiakTS.Date{year: 2014, month: 1, day: 1}, upper: nil}])
  end

  @tag min_pg_version: "9.2"
  test "encode enforces bounds on integer ranges", context do
    # int4's range is -2147483648 to +2147483647
    assert [[%RiakTS.Range{lower: -2147483648}]] = query("SELECT $1::int4range", [%RiakTS.Range{lower: -2147483648}])
    assert [[%RiakTS.Range{upper: 2147483647}]] = query("SELECT $1::int4range", [%RiakTS.Range{upper: 2147483647, upper_inclusive: false}])
    assert %ArgumentError{} = catch_error(query("SELECT $1::int4range", [%RiakTS.Range{lower: -2147483649}]))
    assert %ArgumentError{} = catch_error(query("SELECT $1::int4range", [%RiakTS.Range{upper: 2147483648}]))

    # int8's range is -9223372036854775808 to 9223372036854775807
    assert [[%RiakTS.Range{lower: -9223372036854775807}]] = query("SELECT $1::int8range", [%RiakTS.Range{lower: -9223372036854775807}])
    assert [[%RiakTS.Range{upper: 9223372036854775806}]] = query("SELECT $1::int8range", [%RiakTS.Range{upper: 9223372036854775806, upper_inclusive: false}])
    assert %ArgumentError{} = catch_error(query("SELECT $1::int8range", [%RiakTS.Range{lower: -9223372036854775809}]))
    assert %ArgumentError{} = catch_error(query("SELECT $1::int8range", [%RiakTS.Range{upper: 9223372036854775808}]))
  end
  ...
  test "encode network types", context do
    assert [[%RiakTS.INET{address: {127, 0, 0, 1}}]] =
           query("SELECT $1::inet", [%RiakTS.INET{address: {127, 0, 0, 1}}])
    assert [[%RiakTS.INET{address: {8193, 43981, 0, 0, 0, 0, 0, 0}}]] =
           query("SELECT $1::inet", [%RiakTS.INET{address: {8193, 43981, 0, 0, 0, 0, 0, 0}}])
    assert [[%RiakTS.CIDR{address: {127, 0, 0, 1}, netmask: 32}]] =
           query("SELECT $1::cidr", [%RiakTS.CIDR{address: {127, 0, 0, 1}, netmask: 32}])
    assert [[%RiakTS.CIDR{address: {8193, 43981, 0, 0, 0, 0, 0, 0}, netmask: 128}]] =
           query("SELECT $1::cidr", [%RiakTS.CIDR{address: {8193, 43981, 0, 0, 0, 0, 0, 0}, netmask: 128}])
    assert [[%RiakTS.MACADDR{address: {8, 1, 43, 5, 7, 9}}]] =
           query("SELECT $1::macaddr", [%RiakTS.MACADDR{address: {8, 1, 43, 5, 7, 9}}])
  end
  ...
  test "error struct", context do
    assert {:error, %RiakTS.Error{}} = P.query(context[:pid], "SELECT 123 + 'a'", [])
  end
  ...
  test "prepare, execute and close", context do
    assert (%RiakTS.Query{} = query) = prepare("42", "SELECT 42")
  ...
  test "prepare query and execute different query with same name raise", context do
    assert (%RiakTS.Query{name: "select"} = query42) = prepare("select", "SELECT 42")
    assert close(query42) == :ok
    assert %RiakTS.Query{} = prepare("select", "SELECT 41")
    assert %RiakTS.Error{postgres: %{code: :duplicate_prepared_statement}} =
      execute(query42, [])
  ...
  test "prepare, close and execute", context do
    assert (%RiakTS.Query{} = query) = prepare("reuse", "SELECT $1::int")
  ...
  test "execute prepared query on another connection", context do
    ...
    assert {:ok, %RiakTS.Result{rows: [[42]]}} = RiakTS.execute(pid2, query, [])
    assert {:ok, %RiakTS.Result{rows: [[41]]}} = RiakTS.query(pid2, "SELECT 41", [])
  end
  ...
  test "raise when executing prepared query on connection with different types", context do
    ...
    assert_raise ArgumentError, ~r"invalid types for the connection",
      fn() -> RiakTS.execute(pid2, query, []) end
  end

  test "error codes are translated", context  do
    assert %RiakTS.Error{postgres: %{code: :syntax_error}} = query("wat", [])
  end

  test "connection works after failure in parsing state", context do
    assert %RiakTS.Error{} = query("wat", [])
    assert [[42]] = query("SELECT 42", [])
  end

  test "connection works after failure in binding state", context do
    assert %RiakTS.Error{postgres: %{code: :invalid_text_representation}} =
  ...
  test "connection works after failure in executing state", context do
    assert %RiakTS.Error{postgres: %{code: :unique_violation}} =
  ...
  test "connection works after failure during transaction", context do
    ...
    assert %RiakTS.Error{postgres: %{code: :unique_violation}} =
      query("insert into uniques values (1), (1);", [])
    assert %RiakTS.Error{postgres: %{code: :in_failed_sql_transaction}} =
      query("SELECT 42", [])
  ...
  test "connection works after failure in prepare", context do
    assert %RiakTS.Error{} = prepare("bad", "wat")
    assert [[42]] = query("SELECT 42", [])
  end

  test "connection works after failure in execute", context do
    %RiakTS.Query{} = query = prepare("unique", "insert into uniques values (1), (1);")
    assert %RiakTS.Error{postgres: %{code: :unique_violation}} =
      execute(query, [])
    assert %RiakTS.Error{postgres: %{code: :unique_violation}} =
      execute(query, [])
  ...
  test "connection reuses prepared query after query", context do
    %RiakTS.Query{} = query = prepare("", "SELECT 41")
    assert [[42]] = query("SELECT 42", [])
  ...
  test "connection reuses prepared query after failure in named preparing state", context do
    %RiakTS.Query{} = query = prepare("named", "SELECT 41")
    assert %RiakTS.Error{postgres: %{code: :syntax_error}} = prepare("named", "wat")
    assert [[41]] = execute(query, [])
  end
  ...
  test "raise when trying to execute unprepared query", context do
    assert_raise ArgumentError, ~r/has not been prepared/,
      fn -> execute(%RiakTS.Query{name: "hi", statement: "BEGIN"}, []) end
  end
  ...
  test "raise when trying to prepare or close reserved query", context do
    ...
    query = %RiakTS.Query{query | name: "POSTGREX_BEGIN"}
  ...
  test "query struct interpolates to statement" do
    assert "#{%RiakTS.Query{statement: "BEGIN"}}" == "BEGIN"
  end

  test "connection_id", context do
    assert {:ok, %RiakTS.Result{connection_id: connection_id, rows: [[backend_pid]]}} =
      RiakTS.query(context[:pid], "SELECT pg_backend_pid()", [])
    assert is_integer(connection_id)
    assert connection_id == backend_pid

    assert {:error, %RiakTS.Error{connection_id: connection_id}} =
      RiakTS.query(context[:pid], "FOO BAR", [])
    assert is_integer(connection_id)
  end

  test "empty query", context do
    assert %RiakTS.Result{command: nil, rows: nil, num_rows: 0} =
      RiakTS.query!(context[:pid], "", [])
  end
  ...
  test "COPY FROM STDIN with copy_data: false returns error", context do
    assert %RiakTS.Error{postgres: %{code: :query_canceled}} =
